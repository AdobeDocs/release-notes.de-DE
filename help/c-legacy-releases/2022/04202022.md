---
title: Zentrale Versionshinweise – April 2022
...
| April 2022 | [Informationen zum Umstieg von Client-seitigen Anbieter-Tags auf die Ereignisweiterleitung](https://experienceleague.adobe.com/docs/platform-learn/data-collection/event-forwarding/consider-moving-tags.html?lang=de). | Video | Erfahren Sie, wie Sie ein Client-seitiges Vendor-Tag auswerten, um es möglicherweise in eine Ereignisweiterleitungs-Eigenschaft zu verschieben. |
...
* [Versionshinweise](https://experienceleague.adobe.com/docs/media-analytics/using/additional-resources/release-notes.html) zu [!DNL Streaming Media Analytics] (**neuer Speicherort**)
...
* **Experience Manager Assets as a Cloud Service**
   ...
   * Experience Manager Dynamic Media bietet jetzt die Möglichkeit, in der Benutzeroberfläche [ein (1) Alias-Konto](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/assets/dynamicmedia/dm-alias-account.html) zu konfigurieren, sodass die nativen URLs von [!UICONTROL Dynamic Media] und der Viewer-Einbettungs-Code aktualisiert werden. Diese Aktualisierung wirkt sich positiv auf SEO aus, da Aktualisierungen in Ihrer Geschäftsumgebung, z. B. Rebranding, widergespiegelt werden.
   * Sie können jetzt die Experience Manager Assets-Benutzeroberfläche für folgende Aufgaben verwenden:
      * Konfigurieren der [Erkennung doppelter Medienelemente](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/assets/manage/manage-digital-assets.html#detect-duplicate-assets) in einem Repository
      * Konfigurieren [des Hinzufügens digitaler Wasserzeichen](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/assets/manage/watermark-assets.html) zu Bildern
...
Die nächste Version soll am 10. März 2022 veröffentlicht werden.
   ...
   * Neue beschleunigte [Web Tier Config-Pipelines](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/implementing/using-cloud-manager/cicd-pipelines/introduction-ci-cd-pipelines.html#web-tier-config-pipelines) wurden eingeführt, um ausschließlich HTTPD/Dispatcher-Konfigurationen bereitzustellen.

      * Sie müssen AEM Version `2021.12.6151.20211217T120950Z` oder höher verwenden und [den flexiblen Modus der Dispatcher-Tools auswählen](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/implementing/content-delivery/disp-overview.html#validation-debug), um diese Funktion nutzen zu können.
   ...
   * [Der Zuverlässigkeitsmetrik](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/implementing/using-cloud-manager/test-results/code-quality-testing.html#understanding-code-quality-rules) wurde ein neuer Schwellenwert für Fehlschläge (`< D`) hinzugefügt.

      * Kunden mit schwerwiegenden Qualitätsproblemen, die sich auf die Systemstabilität auswirken und vor allem auf ungültige Indizes und Workflow-Prozesse zurückzuführen sind, können diesen Schwellenwert erst dann bereitstellen, wenn diese Probleme behoben sind.
   * Der Schweregrad der [Qualitätsregel](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/implementing/using-cloud-manager/test-results/code-quality-testing.html#understanding-code-quality-rules) BannedPath wurde von blockierend auf kritisch geändert.
   * Der Pipeline-Assistent informiert den Benutzer, wenn eine Aktualisierung der Experience Manager-Umgebung erforderlich sein könnte, bevor eine damit verbundene [Web Tier Config-Pipeline](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/implementing/using-cloud-manager/cicd-pipelines/introduction-ci-cd-pipelines.html#web-tier-config-pipelines) konfiguriert wird.
...
* **Bevorstehendes Experience Manager GEMs-Webinar**
   ...
   * Thema: _Integration von AEM- und CIF-Framework, um ein eindrucksvolles E-Commerce-Erlebnis zu schaffen_.
      ...
      * Uhrzeit: :00:00 h PDT / 17:0000 h CET / 20:3000 h IST
...
| April 2022 | [Fehlerbehebung in AEM as a Cloud Service mit Repository Browser](https://experienceleague.adobe.com/docs/experience-manager-learn/cloud-service/debugging/debugging-aem-as-a-cloud-service/repository-browser.html?lang=de) | Video | Erfahren Sie mehr über den Repository Browser, ein leistungsstarkes Tool, das Einblick in den zugrunde liegenden Datenspeicher von AEM bietet und einfaches Debugging in der AEM as a Cloud Service-Umgebung ermöglicht. | AEM CS |
...
| April 2022 | [[!DNL Site-Wide Analysis Tool]](https://experienceleague.adobe.com/docs/commerce-operations/tools/site-wide-analysis-tool/intro.html) | Dokumentation | Umfassende Informationen über das Tool [!DNL Site-Wide Analysis], einschließlich seiner Verwendungszwecke, des Installationsprozesses und des Zugriffs darauf.Adobe Commerce |
...
| April 2022 | [Audit-Protokoll](https://experienceleague.adobe.com/docs/campaign-learn/tutorials/monitoring/audit-trail.html) | Video | Erfahren Sie, wie Sie auf das Audit-Protokoll zugreifen und welche Einstellungen konfiguriert werden können. | Campaign v8 |
...
| April 2022 | [Assets Essentials-Überblick](https://experienceleague.adobe.com/docs/journey-optimizer-learn/tutorials/assets-essentials-overview.html) | Video | Erhalten Sie einen Überblick über die Funktionen von Assets Essentials und wie diese in Adobe Journey Optimizer genutzt werden können. |
| April 2022 | [Übersicht über die Journey-Arbeitsfläche](https://experienceleague.adobe.com/docs/journey-optimizer-learn/tutorials/create-journeys/overview-over-the-journey-canvas.html) | Video | Machen Sie sich mit den Funktionen der Journey-Arbeitsfläche vertraut. |
| April 2022 | [Einheitliches Profil und Segmentierung](https://experienceleague.adobe.com/docs/journey-optimizer-learn/tutorials/profiles-segments-subscriptions/unified-profile-and-segmentation-overview.html) | Video | Erfahren Sie, wie Sie ein einheitliches Profil erstellen und anschließend Segmente basierend auf Profilattributen erstellen, um Customer Journeys zu personalisieren. |
...
| April 2022 | [Tags erstellen](https://experienceleague.adobe.com/docs/journey-optimizer-learn/tutorials/decision-management-configuration/create-tags.html) | Video | Erfahren Sie, wie Sie in Offer Decisioning Tags erstellen. Tags sind optionale Bausteinkomponenten von Angeboten. Sie können verwendet werden, um Angebote zu strukturieren und sie in dynamischen Sammlungen zu gruppieren. |
...
Zuletzt aktualisiert: **22. April 2022**, für die Version vom 23. April

| Funktion | Beschreibung |
| ------- | ----------- |
| [!UICONTROL Zielgruppen] | ([!DNL Microsoft Advertising]-Konten, die für den Kundenabgleich in Frage kommen) Sie können jetzt Zielgruppen für den Kundenabgleich erstellen und verwalten, indem Sie CSV-Dateien mit E-Mail-Adressen hochladen. Die Daten müssen mit dem SHA-256-Algorithmus gehasht werden. |
| [!UICONTROL Kampagnen] | Die folgenden Beta-Funktionen stehen Advertisern zur Verfügung, die sich anmelden:<ul><li>([!DNL Microsoft Advertising]-Konten) Unterstützung für Synchronisierung, schreibgeschützte Anzeige und Reporting (einschließlich View-Through-Daten) für Ihre vorhandenen nativen Anzeigenkampagnen auf dem [!DNL Microsoft Audience Network], einschließlich [!DNL Microsoft Audience Ads].</li><li>([!DNL Google Ads]- und [!DNL Microsoft Advertising]-Konten) Möglichkeit zum Importieren Ihrer [!DNL Google Ads]-Kampagnen und Kampagnenstruktur über Advertising Cloud Search nach [!DNL Microsoft Advertising].</li></ul>Wenden Sie sich an Ihren Adobe-Kundenbetreuer, wenn Sie an einem der Betaprogramme interessiert sind. |
| [!UICONTROL Kampagnen]<br><br>[!UICONTROL Portfolios] | ([!DNL Microsoft Advertising]-Kampagnen) Unterstützung für die folgenden Gebotsstrategien ist verfügbar:<ul><li>(Allgemeine Verfügbarkeit) [!UICONTROL Maximieren von Konversionen], [!UICONTROL Ziel-CPA] und [!UICONTROL Ziel-Rendite auf Werbeausgaben]: Diese Strategien stehen nun allen Benutzern zur Verfügung. Sie können Suchkampagnen zu dieser Gebotsstrategie hinzufügen und Hybridportfolios (nicht aber Standardportfolios) erstellen.</li><li>(Beta-Funktion) [!UICONTROL Target Impression Share]: Wenn Sie die Betaversion zur Hybridoptimierung verwenden, können Sie Kampagnen mit dieser Strategie konfigurieren und optional einen Ziel-Impressionsanteil, eine Ziel-Anzeigenposition und Maximalkosten pro Klick festlegen. Achtung: Diese Option wird in Hybridportfolios noch nicht unterstützt und kann nicht zu Standardportfolios hinzugefügt werden.</li><li>(Beta-Funktion) [!UICONTROL Klicks maximieren]: Wenn Sie die Betaversion zur Hybridoptimierung verwenden, können Sie Kampagnen mit dieser Strategie konfigurieren und optional ein Ziel für die Maximalkosten pro Klick festlegen. Mit dieser Strategie können Sie Kampagnen entweder in Standard- oder Hybridportfolios einbeziehen. Um diese Strategie in einem Hybridportfolio zu verwenden, dürfen im Ziel des Portfolios nur [!DNL Adobe]-Eigenschaften (Metriken) vorhanden sein. Zusätzlich müssen Sie das Hochladen von Advertising Cloud Search-Zielen nach [!DNL Microsoft Ads] aktivieren.</li></ul>Wenn Sie die Betaversion zur Hybridoptimierung noch nicht verwenden und an der Beta-Phase teilnehmen möchten, wenden Sie sich an Ihren [!DNL Adobe] Account Manager. |
| [!UICONTROL Advertising Insights] | Alle Insights außer [!UICONTROL Query Cross Matching] und [!UICONTROL Hybrid Portfolio Setup Readiness] sind jetzt nicht mehr in der Beta-Phase. |
...
| Integration in Adobe Analytics | (7. April) Im Daten-Feed, den Advertising Cloud an [!DNL Analytics] sendet, werden Daten für [!DNL Google Ads] und Search Ads (RSAs) von [!DNL Microsoft Advertising] mit dem [!UICONTROL Responsive Ad-Typ] „[!UICONTROL Responsive Search Ads]“ neu klassifiziert, wenn neue Klicks verzeichnet werden. Zuvor waren die Daten für den [!UICONTROL Ad-Typ] „[!UICONTROL Text Ads]“ enthalten. RSAs in inaktiven Kampagnen werden nicht neu klassifiziert.<br><br>Für neu klassifierte RSAs wird der [!UICONTROL Responsive Ad-Titel] jetzt mit der ersten Überschrift und die [!UICONTROL Responsive Ad-Beschreibung] mit der ersten Beschreibung gefüllt. |
...
| April 2022 | [Ändern eines Dokuments nach dem Versand](https://experienceleague.adobe.com/docs/document-cloud-learn/sign-learning-hub/getting-started/getting-started-sending/modify-in-flight.html) | Video | Erfahren Sie, wie Sie ein Dokument ändern, das bereits zur Unterschrift verschickt wurde, beispielsweise wenn versehentlich das falsche Dokument verschickt wurde. | Adobe Sign |
| April 2022 | [Einrichten der Signierreihenfolge](https://experienceleague.adobe.com/docs/document-cloud-learn/sign-learning-hub/advanced-tasks/advanced-tasks-sending/setting-up-routing.html) | Video | Erfahren Sie, wie Sie die Signierreihenfolge für mehrere Unterzeichner einrichten können. Senden Sie Dokumente nacheinander und/oder gleichzeitig oder an bestimmte Personengruppen. | Adobe Sign |
